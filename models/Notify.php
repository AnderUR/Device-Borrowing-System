...
class notify extends CI_Model
{
    ...
    function email_user($uid, $senderMsg, $subject = false)
    {
        ...
        if (isset($user)) {
            $this->email->clear();
            $this->email->from($this->config->item('admin_email', 'ion_auth'), 'DBS');
            $this->email->to($user['email']);
            $msg = str_replace('%20', ' ', $senderMsg);
            $message = $msg;
            if ($subject == false) {
                $this->email->subject("DBS Account Information");
            } else {
    ...
    function emailDeviceLoanReceipt($email, $senderMsg, $subject=false) {
        $this->email->clear();
        $this->email->from($this->config->item('admin_email', 'ion_auth') , 'DBS');
        $this->email->to($email);
        $msg = str_replace('%20', ' ', $senderMsg);
        $message = $msg;
        if ($subject == false){
            $this->email->subject("Device Borrowing System");
        }else{
            $this->email->subject($subject);
        }
        $this->email->message($message);
